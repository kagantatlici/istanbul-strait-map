...
console.log('🔌 WebSocket server configured for root and /ais paths');
console.log('🌐 WebSocket endpoint available at: ws://localhost:' + PORT + '/ (root)');
...
wss.on('connection', (ws, req) => {
    ...
    console.log(`🔌 New WebSocket connection established on ${connectionPath} path`);
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (data.type === 'request_ships') {
                console.log('📋 Client requested current ships on', connectionPath, 'path, sending', aisProxy.shipData.size, 'ships');
    ...
    ws.on('close', () => {
        console.log('🔌 WebSocket connection closed on', connectionPath, 'path');
        aisProxy.removeClient(ws);
    });

    ws.on('error', (error) => {
        console.error('❌ WebSocket client error on', connectionPath, 'path:', error);
        aisProxy.removeClient(ws);
    });
    ...
    setTimeout(() => {
        if (ws.readyState === WebSocket.OPEN) {
            console.log('📤 Sending initial status to new client on', connectionPath, 'path');
            ws.send(JSON.stringify({
                type: 'status_update',
                status: 'connected',
            ...
            // Send current ships to new client
            console.log(`📤 Sending ${aisProxy.shipData.size} current ships to new client on ${connectionPath} path`);
            aisProxy.shipData.forEach(ship => {
                if (ws.readyState === WebSocket.OPEN) {
                    ws.send(JSON.stringify({
                        type: 'ship_update',
